...
# ---------------------------------------------------------
# 2. Sidebar API Key Input (Protects key from revocation)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 4. Backend Python Function for Groq API Call
# ---------------------------------------------------------
def call_groq_api():
...
with col1:
    if st.button("▶️ Start Interview", use_container_width=True, disabled=st.session_state.interview_started):
        st.session_state.interview_started = True
        with st.spinner("Panelist is preparing..."):
            call_groq_api()
        st.rerun()

user_transcript = st.text_input("🎤 Your Spoken Answer (Type or use browser Speech-to-Text):", placeholder="Type or dictate your answer here...")

with col3:
    if st.button("🧠 Submit Answer", use_container_width=True, disabled=not st.session_state.interview_started):
        if user_transcript.strip():
            st.session_state.history.append({"role": "user", "content": user_transcript.strip()})
        else:
            st.session_state.history.append({"role": "user", "content": "Candidate did not provide a clear response."})
...
# ---------------------------------------------------------
# 6. Interactive 3D Avatar + Lip Sync HTML Component
# ---------------------------------------------------------
...
<body class="p-2">
    ...
    <div class="max-w-4xl mx-auto space-y-4">
        ...
        <div class="glass p-6 rounded-xl space-y-2 border border-blue-500/20">
    ...
    <script>
        ...
        // --- 1. Voice Playback (Speech Synthesis) ---
...
components.html(app_html, height=520, scrolling=False)
