...
# Auto-Process spoken answer when URL query param changes
...
# ---------------------------------------------------------
# 6. Interactive 3D Avatar + Auto-Submit Voice Interface
# ---------------------------------------------------------
    ...
    <script>
        ...
        if (SpeechRecognition) {{
            ...
            recognition.onresult = (e) => {{
                ...
                // Silence detection: If user pauses for 2.2 seconds, automatically submit
            ...
            recognition.onend = () => {{
                // Keep mic open if panelist isn't currently speaking
                if (!isSpeaking && {q_count} <= 5) {{
        ...
        // --- Voice Playback & Automatic Mic Handoff ---
        function speakText(text) {{
            ...
            msg.onend = () => {{
                ...
                // Automatically turn on microphone once panelist finishes speaking
                if (recognition && {q_count} <= 5) {{
                    try {{ recognition.start(); }} catch(e) {{}}
                }}
            }};
