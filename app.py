That infinite loop happens because Streamlit re-renders the whole page whenever the URL parameters change, which can trigger the Web Speech API's onresult event repeatedly before the page finishes reloading.
Here is the fixed app.py that fixes this glitch once and for all by:
 1. **Adding a submission lock (isSubmitting)** in JavaScript so it can only send your audio transcript **once** per answer.
 2. **Stopping the speech recognizer immediately** as soon as silence is detected to prevent ghost inputs.
 3. **Handling the transcript state safely on the Python side** to ensure the panelist processes your answer smoothly without getting stuck on "Thinking...".
### 📄 Updated app.py Code
Replace your entire **app.py** on GitHub with this code:
```python
